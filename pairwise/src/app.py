...
def server(input, output, session):
	...
	Info = {
		"example1.txt": "<u>Input type:</u> .txt Data <br><u>Contents:</u> A pre-computed matrix of pairwise distances between C-alpha atoms in ubiquitin (1ubq).",
		#"example2.txt": "<u>Input type:</u> .txt Data <br><u>Contents:</u> This example dataset was generated randomly.",
		"example3.txt": "<u>Input type:</u> .txt Data<br><u>Contents:</u> This example dataset contains a pre-computed matrix that was generated randomly. If you are new to Heatmapper2, try playing with the settings on this dataset. Its small size means that setting changes are visualized nearly instantly.",
		"example4.fasta": "<u>Input type:</u> .FASTA file <br><u>Contents:</u> This file contains sample sequences in FASTA format. The first sequence is bovine prolactin precursor, the second sequence is human/rabbit/bovine/rat/chicken calmodulin, and the third sequence is Elephas maximus maximus cytochrome b.",
		"ala_phe_ala.pdb": '<u>Input type:</u> .PDB file <br><u>Contents:</u> This PDB file contains an unnamed compound composed of Alanine, Phenylalanine, Alanine. <br><u>Source:</u> Generated by Open Babel 2.3.2',
		"city_geospatial_info.tsv": '<u>Input type:</u> .tsv Data <br><u>Contents:</u> This data contains x, y, and z columns, so can be visualized as a cube matrix. The x and y columns contain the longitude and latitude of cities in Africa, and the z column is the elevation at those coordinates. The value column contains the city name. <br><u>Source:</u> <a href="https://en.wikipedia.org/wiki/List_of_cities_by_elevation"; target="_blank">Wikipedia</a>'
	}
	...
	def Welcome():
		return ui.HTML("""
			...
			<b>1 - PDB Files</b><br>
			Upload a <b>.pdb</b> file to compute a matrix using coordinates between atoms. Use different atoms by specifying a different chain under 'Chain' in the sidebar (see example 'Ex4: PDB').<br><br>

			<b>2 - FASTA Files</b><br>
		    Upload a <b>.fasta</b> file. Heatmapper will parse the file and partition the sequence(s) into K-Mers. A distance or correlation matrix is generated based on the counts of each K-Mer. (see example 'Ex3: FASTA').<br><br>

			<b>3 - Table Files</b><br>
			Table files with a 'Name', 'X'', 'Y', and optional 'Z' column will be turned into a matrix, labelled with the 'Name' column. Including a 'Z' column allows the data to be displayed as a cube matrix (see example Ex5: CubeMatrix).<br>
...
app_ui = ui.page_fluid(
	...
	ui.layout_sidebar(
		ui.sidebar(
			...
			FileSelection(
				examples={
				"example1.txt": "Ex1: Pre-Computed Matrix",
				#"example2.txt": "Ex2: RandomData",
				"example3.txt": "Ex2: Small Matrix",
				"example4.fasta": "Ex3: FASTA",
				"ala_phe_ala.pdb": "Ex4: PDB",
				"city_geospatial_info.tsv": "Ex5: CubeMatrix",
				},
