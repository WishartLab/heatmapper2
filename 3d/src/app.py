	...
	def ConditionalElements():
		...
		if type(data) == str or input.SourceFile() == "ID":
			elements += [
				...
				config.Thickness.UI(ui.input_numeric, id="Thickness", label="Cartoon Thickness", min=0, max=10, step=0.1, tooltip="Cartoon style only - specify the thickness of the visualized components. Lower values make components thinner, while higher values (to a maximum of 10) make components thicker."),
				config.Width.UI(ui.input_numeric, id="Width", label="Cartoon Width", min=0, max=10, step=0.1, tooltip="Cartoon style only - specify the width of the visualized components. Lower values make components narrower, while higher values (to a maximum of 10) make components wider."),
				config.Radius.UI(ui.input_numeric, id="Radius", label="Atom Radius", min=0, max=5, step=0.05, tooltip="Stick, Sphere, or Cross style only - Specify a fixed radius (in Angstroms) for atoms in the <i>model</i>. Lower values make atoms smaller, while higher values (to a maximum of 5) make atoms larger. This value overrides 'Scale'."),
